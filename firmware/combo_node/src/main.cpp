...
#ifndef LED_BUILTIN
#define LED_BUILTIN 2     // onboard LED on most ESP32 dev boards (active-HIGH)
#endif
...
bool readPMS(float &pm25, float &pm10) {
  ...
  while (millis() - start < 1500) {
    if (Serial2.available() && Serial2.read() == 0x42) {
      if (Serial2.read() != 0x4D) continue;
      buf[0] = 0x42; buf[1] = 0x4D;
      if (Serial2.readBytes(buf + 2, 30) != 30) continue;
      uint16_t sum = 0;
...
bool readSDS(float &pm25, float &pm10) {
  ...
  unsigned long start = millis();
  while (millis() - start < 1500) {
    if (Serial2.available() && Serial2.read() == 0xAA) {
      buf[0] = 0xAA;
      if (Serial2.readBytes(buf + 1, 9) == 9 && buf[1] == 0xC0 && buf[9] == 0xAB) {
...
void setup() {
  ...
  if (cfg.pms || cfg.sds) Serial2.begin(cfg.pms ? 9600 : 9600, SERIAL_8N1, PMS_RX, PMS_TX);
  if (cfg.bmp) {
